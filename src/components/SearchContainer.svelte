<script lang="ts">
  ...
  const searchNotices = async () => {
    try {
			...
			if (firstName)
				params.append("forename", firstName.toUpperCase());
			if (lastName) params.append("name", lastName.toUpperCase());
			if (country)
				params.append("nationality", country);
...
<div class="flex flex-col items-center justify-center gap-3 p-4 text-white">
  <p class="text-xl sm:text-xl md:text-xl font-bold tracking-wide underline decoration-sky-500">Criminals Search</p>
  <div class="search-container">
    ...
    <div class="sex flex gap-3">
      Sex:
      <label>
        <input type="radio" class="focus:ring-blue-800" bind:group={sex} value="M" /> Male
      </label>
      <label>
        <input type="radio" class="focus:ring-blue-800" bind:group={sex} value="F" /> Female
      </label>
    </div>
  
    <button class="px-3 py-2 text-sm font-medium text-center text-white rounded-lg focus:ring-4 focus:outline-none bg-blue-600 hover:bg-blue-700 focus:ring-blue-800 mt-7"
