  ...
  class Criminal {
    ...
    async fetchCriminalData(href: string) {
      ...
      if (this.distinguishingMarks) {
        this.distinguishingMarks = data
          .distinguishing_marks!.split(" ")
          .map(
            (mark) => mark.charAt(0).toUpperCase() + mark.slice(1).toLowerCase()
          )
          .join(" ");
      }
      ...
      this.imagesLinks = imagesData._embedded.images.map(
        (image) => image._links.self.href
      );
    }
  }
...
<div class="w-full">
  {#if dataFetched}
    <h1 class="text-3xl sm:text-4xl md:text-5xl text-center text-white mb-5">
      {criminal.name + " " + criminal.forename}
    </h1>
    <div
      class="flex flex-col md:flex-row justify-center gap-5 p-4 text-white w-full"
    >
      <aside class="w-full md:w-1/3 flex flex-col gap-2 p-4 font-bold">
        ...
        <div class="flex items-center gap-x-3 flex-wrap">
          ...
          {#each criminal.nationalities as nationality}
            <div class="flex">
              ...
              />
              <span
                class="text-white text-xs rounded py-1 whitespace-nowrap ml-1"
              >
        ...
        <p>
          Date of birth: <span class="font-normal">{criminal.dateOfBirth}</span>
        </p>
