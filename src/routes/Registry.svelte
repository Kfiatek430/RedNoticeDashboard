<script lang="ts">
    ...
    async function fetchNotices() {
        ...
        const notices: BasicNotice[] = data._embedded.notices;

        const criminalPromises = notices.map(async (element) => {
            let criminal = new Criminal(element._links.self.href);
            await criminal.run();
            return criminal;
        });

        criminals = await Promise.all(criminalPromises);
    }
...
<div class="flex justify-center gap-6 flex-wrap p-4">
    {#each criminals as criminal}
        <Card
            ...
            description={criminal.arrestWarrants[0].charge || "No charge available"}
            profilePicturePath={criminal.profilePicturePath}
            link={criminal.externalLink}
        />
    {/each}
</div>
